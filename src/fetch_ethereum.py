...
def fetch_ethereum_blocks(num_blocks=10):
    ...
    for block_num in range(latest_block_int, latest_block_int - num_blocks, -1):
        ...
        block_url = f"{BASE_URL}?module=proxy&action=eth_getBlockByNumber&tag={hex_block_num}&boolean=true&apikey={ETHERSCAN_API_KEY}"
        ...
        block_fees_info.append({
            'height': block_num,
            'fee_total_btc': block_fee_total,  # ETH but using BTC key for plotting
            'tx_count': tx_count
        })

        print(f"✅ Ethereum Block {block_num}: {tx_count} TXs, {block_fee_total:.6f} ETH fees")
        sleep(0.2)

    print(f"\n📦 Ethereum: {num_blocks} blocks, {len(all_transactions)} transactions")
